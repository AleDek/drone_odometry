    ...
    : nh_(nh), nh_private_(nh_private), initialized_(false)
{
    ...
    //custom
    double yaw_zero = 0.00f;
    double roll_zero = 0.00f;
    double pitch_zero = 0.00f;
    if (!nh_private_.getParam("yaw_zero", yaw_zero)) yaw_zero = 0.00f;
    if (!nh_private_.getParam("roll_zero", roll_zero)) roll_zero = 0.00f;
    if (!nh_private_.getParam("pitch_zero", pitch_zero)) pitch_zero = 0.00f;
    initial_orientation_.setRPY(roll_zero, pitch_zero,  yaw_zero);
    //end custom
...
void ImuFilterRos::imuCallback(const ImuMsg::ConstPtr& imu_msg_raw)
{
    ...
    {   
        ...
        //custom
        init_q.x = initial_orientation_.x();
        init_q.y = initial_orientation_.y();
        init_q.z = initial_orientation_.z();
        init_q.w = initial_orientation_.w();
        ...
        //filter_.setOrientation(init_q.w, init_q.x, init_q.y, init_q.z); //custom
    }
